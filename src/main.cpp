...
#if defined(NDEBUG)
# error "Legendcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa105b93f8c52645945e13b4485e3110d71f3141a2981b0f4e957317893f7b90a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Legendcoin: starting difficulty is 1 / 2^12
...
{
    // Legendcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Legendcoin: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Legendcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Legendcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Legendcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdd56a1782733f65cc7962a4445163fe064d7a09fc60346333003e3454513757d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "LegendCoin LGC";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1399638115;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 388446624;

        if (fTestNet)
        {
            block.nTime    = 1399551909;
            block.nNonce   = 385839198;
        }
        ...
        if (block.GetHash() != hashGenesisBlock)
        {
           ...
           while(true)
           {
               ...
               {
                   //qDebug("nonce %08X: hash = %s (target = %s)\n", block.nNonce, thash.ToString().c_str(), hashTarget.ToString().c_str());
                   printf("nonce %08X: hash = %s (target = %s)\n", block.nNonce, thash.ToString().c_str(), hashTarget.ToString().c_str());
               }
               ++block.nNonce;
               if (block.nNonce == 0)
               {
                   //qDebug("NONCE WRAPPED, incrementing time\n");
        ...
        assert(block.hashMerkleRoot == uint256("0x04258111ffad5ce8fe2f329803762c0de427706e245fb116f8b06b3460a3f249"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Legendcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LegendcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LegendcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LegendcoinMiner(CWallet *pwallet)
{
    printf("LegendcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("legendcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LegendcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LegendcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LegendcoinMiner, pwallet));
}
